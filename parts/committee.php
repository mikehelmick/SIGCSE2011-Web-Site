  
	<table class="FadeTable" cellspacing="0" cellpadding="0">
	  <tr class="header"><td colspan="3" align="center">PROGRAM COMMITTEE</td></tr> 
	  <tr><td align="left" valign="top">
	<p><strong>Symposium Chairs</strong><br /> 
	Thomas Cortina<br /> 
	Carnegie Mellon University<br /> 
	&nbsp;<br/>
	Ellen Walker<br /> 
	Hiram College<br />
	<script>write_email( "sigcse11-symposium", "cs.cmu.edu" );</script>
	</p>

	<p>
	<strong>Program Chairs</strong><br>
	Laurie Smith King<br/>
	College of the Holy Cross<br />
	&nbsp;<br/>
	Dave Musicant <br/>
	Carleton College<br/>
	<script>write_email( "sigcse11-program", "cs.cmu.edu" );</script></p> 

	<p><b>Panels and Special Sessions</b><br/>
	Pam Cutter<br/>
	Kalamazoo College<br />
	<script>write_email( "sigcse11-panels", "cs.cmu.edu" );</script><br />
    <script>write_email( "sigcse11-specials", "cs.cmu.edu" );</script></p>

	<p><b>Workshops</b><br/>
	Ruth E. Anderson <br/>
	University of Washington <br/>
	&nbsp;<br/> 
	Adrienne Decker<br/>
	University at Buffalo (SUNY)<br />
	<script>write_email( "sigcse11-workshops", "cs.cmu.edu" );</script></p>

	<p><b>Publications</b><br/>
	Lester I. McCann <br/>
	The University of Arizona <br/>
	<script>write_email( "sigcse11-publications", "cs.cmu.edu" );</script></p>

	<p><b>Registration</b><br/>
	Cary Laxer and Lynn Degler<br/>
	Rose-Hulman Institute of Technology<br/>
	&nbsp;<br/>
	Larry Merkle<br/>
	Wright State University<br/>
	<script>write_email( "sigcse11-registration", "cs.cmu.edu" );</script></p>

	</td><td align="left" valign="top">

	<p><b>Posters</b><br/>
	Tzu-Yi Chen<br />
    Pomona College<br />
	<script>write_email( "sigcse11-posters", "cs.cmu.edu" );</script></p>

	<p><b>Birds Of A Feather</b><br/>
	Deborah Hwang<br/>
	University of Evansville
	<script>write_email( "sigcse11-bofs", "cs.cmu.edu" );</script></p>

	<p><b>Videos Coordinator</b><br/>
	Dennis Bouvier <br/>
	Southern Illinois University - Edwardsville <br/>
	<script>write_email( "sigcse11-videos", "cs.cmu.edu" );</script></p>

	<p><b>Student Volunteers and Student Activities</b><br/>
	Brianna Morrison <br/>
	Southern Polytechnic State University <br/>
	&nbsp;<br/>
	Mary Anne Egan<br/>
	Siena College<br/>
	<script>write_email("sigcse11-volunteers", "cs.cmu.edu");</script></p>

	<p><b>Treasurer</b><br/>
	Scott Grissom<br/>
	National Science Foundation &amp; <br/>Grand Valley State University<br/>
	<script>write_email("sigcse11-treasurer", "cs.cmu.edu");</script></p>

	<p><b>Database Administrators</b><br/>
	Henry Walker <br/>
	Grinnell College <br/>
	<br/> 
	John Dooley <br/>
	Knox College <br/>
	<script>write_email( "sigcse11-database", "cs.cmu.edu" );</script></p>
	
	<p><b>Webmaster</b><br/>
	Michael T. Helmick <br/>
	Google <br/>
	<script>write_email( "sigcse11-webmaster", "cs.cmu.edu" );</script></p>

    </td><td align="left" valign="top">

	<p><b>Evaluations</b><br/>
	Kurt Eiselt <br/>
	University of British Columbia <br/>
	<script>write_email( "sigcse11-evaluations", "cs.cmu.edu" );</script></p>

	<p><b>Kids' Camp</b><br/>
	Kimberly Voll<br />
	University of British Columbia<br />
	&nbsp;<br/>
	RoxAnn Stalvey<br/>
	College of Charleston<br/>
	<script>write_email( "sigcse11-kidscamp", "cs.cmu.edu" );</script><br/>
	</p>

	<p><b>Support/Exhibitor Liaison</b><br/>
	Susan Rodger<br/>
	Duke University<br />
	<script>write_email( "sigcse11-exhibitor", "cs.cmu.edu" );</script></p>

	<p><b>K-12 Liaisons</b><br/>
	Stacey Armstrong <br/>
	Cypress Woods High School,<br/>
	Houston, Texas<br/>
	&nbsp;<br/>
	Judy Hromchik<br/>
	Arlington High School<br/>
	Arlington, Texas<br/>
	<script>write_email( "sigcse11-k12", "cs.cmu.edu" );</script>
	</p>

	<p><b>International Liaison</b><br/>
	Tim Bell <br/>
	University of Canterbury<br/>
	<script>write_email( "sigcse11-international", "cs.cmu.edu" );</script></p>

	<p><b>Pre-Conference Event Liaison</b><br />
    Kristine Nagel<br />
    Georgia Gwinnett College<br />
    <script>write_email( "sigcse11-preconference", "cs.cmu.edu" );</script></p>

	<p><b>Local Arrangements</b><br/>
	Jian Zhang<br/>
	Texas Women's University<br/>
	<script>write_email( "jzhang", "mail.twu.edu" );</script></p>

	<p><b>Student Research Competition</b><br/>
	Ann Sobel <br/>
	Miami University (Ohio) <br/>
	<script>write_email( "sigcse11-src", "cs.cmu.edu" );</script></p>
	</td></tr>
	</table>