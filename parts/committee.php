  
	<table class="FadeTable" cellspacing="0" cellpadding="0">
	  <tr class="header"><td colspan="3" align="center">PROGRAM COMMITTEE</td></tr> 
	  <tr><td align="left" valign="top">
	<p><strong>Symposium Chairs</strong><br /> 
	Thomas Cortina<br /> 
	Carnegie Mellon University<br /> 
	<script>write_email( "tcortina", "cs.cmu.edu" );</script>
	&nbsp;<br/>
	Ellen Walker<br /> 
	Hiram College<br />
	<script>write_email( "walkerel", "hiram.edu" );</script>
	</p>

	<p>
	<strong>Program Chairs</strong><br>
	Laurie Smith King<br/>
	College of the Holy Cross<br />
	<script>write_email( "lking", "holycross.edu" );</script>
	&nbsp;<br/>
	Dave Musicant <br/>
	Carleton College<br/>
	<script>write_email( "dmusican", "carleton.edu" );</script></p> 

	<p><b>Panels and Special Sessions</b><br/>
	Pam Cutter<br/>
	Kalamazoo College
	<script>write_email( "Pam.Cutter", "kzoo.edu" );</script></p>

	<p><b>Workshops</b><br/>
	Ruth E. Anderson <br/>
	University of Washington <br/>
	<script>write_email( "rea", "cs.washington.edu" );</script>
	&nbsp;<br/> 
	Adrienne Decker<br/>
	University at Buffalo (SUNY)
	<script>write_email( "adrienne", "buffalo.edu" );</script></p>

	<p><b>Publications</b><br/>
	Lester I. McCann <br/>
	The University of Arizona <br/>
	<script>write_email( "mccann", "cs.arizona.edu" );</script></p>

	<p><b>Registration</b><br/>
	Cary Laxer and Lynn Degler<br/>
	Rose-Hulman Institute of Technology<br/>
	&nbsp;<br/>
	Larry Merkle<br/>
	Wright State University<br/>
	<script>write_email( "sigcse2011", "csse.rose-hulman.edu" );</script></p>

	</td><td align="left" valign="top">

	<p><b>Posters</b><br/>
	Tzu-Yi Chen<br />
    Pomona College<br />
	<script>write_email( "tzuyi", "cs.pomona.edu" );</script></p>

	<p><b>Birds Of A Feather</b><br/>
	Deborah Hwang<br/>
	University of Evansville
	<script>write_email( "hwang", "evansville.edu" );</script></p>

	<p><b>Videos Coordinator</b><br/>
	Dennis Bouvier <br/>
	Southern Illinois University - Edwardsville <br/>
	<script>write_email( "dbouvie", "siue.edu" );</script></p>

	<p><b>Student Volunteers and Student Activities</b><br/>
	Brianna Morrison <br/>
	Southern Polytechnic State University <br/>
	<script>write_email( "bmorriso", "spsu.edu" );</script><br/>
	&nbsp;<br/>
	Mary Anne Egan<br/>
	Siena College<br/>
	<script>write_email("maegan","siena.edu");</script></p>

	<p><b>Treasurer</b><br/>
	Scott Grissom<br/>
	National Science Foundation &amp; <br/>Grand Valley State University<br/>
	<script>write_email("grissom","gvsu.edu");</script></p>

	<p><b>Database Administrators</b><br/>
	Henry Walker <br/>
	Grinnell College <br/>
	<script>write_email( "walker", "cs.grinnell.edu" );</script><br/>
	<br/> 
	John Dooley <br/>
	Knox College <br/>
	<script>write_email( "jdooley", "knox.edu" );</script></p>
	
	<p><b>Webmaster</b><br/>
	Michael T. Helmick <br/>
	Amazon.com <br/>
	<script>write_email( "mike.helmick", "gmail.com" );</script></p>

    </td><td align="left" valign="top">

	<p><b>Evaluations</b><br/>
	Kurt Eiselt <br/>
	University of British Columbia <br/>
	<script>write_email( "eiselt", "cs.ubc.ca" );</script></p>

	<p><b>Kids' Camp</b><br/>
	Kimberly Voll<br />
	University of British Columbia<br />
	<script>write_email( "kvoll", "cs.ubc.ca" );</script><br/>
	&nbsp;<br/>
	RoxAnn Stalvey<br/>
	College of Charleston<br/>
	<script>write_email( "stalveyr", "cofc.edu" );</script><br/>
	</p>

	<p><b>Support/Exhibitor Liaison</b><br/>
	Susan Rodger<br/>
	Duke University<br />
	<script>write_email( "rodger", "cs.duke.edu" );</script></p>

	<p><b>K-12 Liaisons</b><br/>
	Stacey Armstrong <br/>
	Cypress Woods High School,<br/>
	Houston, Texas<br/>
	<script>write_email( "stacey.armstrong", "apluscompsci.com" );</script><br/>
	&nbsp;<br/>
	Judy Hromchik<br/>
	Arlington High School<br/>
	Arlington, Texas<br/>
	<script>write_email( "judy.hromcik", "att.net" );</script>
	</p>

	<p><b>International Liaison</b><br/>
	Tim Bell <br/>
	University of Canterbury<br/>
	<script>write_email( "tim.bell", "canterbury.ac.nz" );</script></p>

	<p><b>Pre-Conference Event Liaison</b><br />
    Kristine Nagel<br />
    Georgia Gwinnett College<br />
    <script>write_email( "knagel", "ggc.edu" );</script></p>

	<p><b>Local Arrangements</b><br/>
	Jian Zhang<br/>
	Texas Women's University<br/>
	<script>write_email( "jzhang", "mail.twu.edu" );</script></p>

	<p><b>Student Research Competition</b><br/>
	Ann Sobel <br/>
	Miami University (Ohio) <br/>
	<script>write_email( "sobelae", "muohio.edu" );</script></p>
	</td></tr>
	</table>